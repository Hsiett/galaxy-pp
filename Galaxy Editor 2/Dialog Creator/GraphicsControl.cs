...
//using CompilerOptions = Galaxy_Editor_2.Options.CompilerOptions;
//using Font = Microsoft.Xna.Framework.Graphics.fon
using Microsoft.Xna.Framework.Content.Pipeline.Processors;
using EMC = Microsoft.Xna.Framework.Content.Pipeline.Graphics.EffectMaterialContent;
...
namespace Galaxy_Editor_2.Dialog_Creator
{
    class GraphicsControl : GraphicsDeviceControl
    {
        ...
        static void InitStatic(GraphicsControl control)
        {
            ...
            var effect = ep.Process(new EffectContent { EffectCode = code }, new CatalogContext());
        ...
        protected override void Initialize()
        {
            ...
            renderTarget = new RenderTarget2D(GraphicsDevice, (int) (GraphicsDevice.DisplayMode.Width/scale), 
                        (int) (GraphicsDevice.DisplayMode.Height/scale), false, SurfaceFormat.Color,DepthFormat.Depth24Stencil8);
        ...
        protected override void DeviceReset()
        {
          ...
          //  DepthStencilBuffer oldStencil = depthStencilBuffer;
            ...
            {
                renderTarget = new RenderTarget2D(GraphicsDevice, (int)(GraphicsDevice.DisplayMode.Width / scale), 
                                (int)(GraphicsDevice.DisplayMode.Height / scale), true, SurfaceFormat.Color,DepthFormat.Depth24Stencil8);
              //  depthStencilBuffer = CreateDepthStencil(renderTarget);
            }
            catch
            {
        ...
        private string textsizefix(string s, SpriteFont font, float textScale, Rectangle rect)
        {
            if (s.Length == 0)
                return s;
            string ret;
            Vector2 size = font.MeasureString(s)* textScale;
            if(size.Y> rect.Height)//show .. if apart from y,g
            {
                s = "...";   
            }
            //size = font.MeasureString(s) * 0.7f;
            while (size.X > rect.Width)//string is too long to fit into the document, remain the parts that is inside -1 plus .
            {
                int remain = rect.Width/((int)((size.X) / (s.Length)));
                if (remain >= 5)
                    s = s.Substring(0, 2) + "...";
                else if (remain >= 3)
                    s = "...";
                    ...
                    s = ".";
                size = font.MeasureString(s) * textScale;
            }
            return s;
        }
        ...
        private class CatalogContext : ContentProcessorContext
        {
